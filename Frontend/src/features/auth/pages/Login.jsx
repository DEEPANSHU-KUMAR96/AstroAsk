...
const Login = () => {
  ...
  return (
    <div style={{ height: "100vh", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", background: "#fbf9f8" }}>
      <AtmosphericBg />
      <div className="glass-panel" style={{ width: "100%", maxWidth: "440px", borderRadius: "12px", padding: "clamp(16px, 2.8vh, 30px) clamp(20px, 2.8vw, 38px)", position: "relative", overflow: "hidden", zIndex: 1, margin: "0 16px" }}>
        ...
        <div style={{ marginBottom: "clamp(12px, 2vh, 20px)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
            ...
            <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(22px, 3.2vh, 32px)", fontWeight: 700, color: "#1a1a1a", lineHeight: 1.15, margin: 0 }}>Welcome Back</h1>
          </div>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "12px", lineHeight: 1.45, color: "#514532", margin: 0 }}>Sign in to continue your cosmic journey and view your alignments.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate style={{ display: "flex", flexDirection: "column", gap: "clamp(8px, 1.4vh, 14px)" }}>
          ...
          <div style={{ paddingTop: "clamp(2px, 0.6vh, 6px)" }}>
            <button type="submit" disabled={loading} className="btn-gold" style={{ width: "100%", padding: "12px 20px" }} aria-busy={loading}>
              {loading ? (
                <><span style={{ display: "inline-block", width: "13px", height: "13px", border: "2px solid rgba(26,26,26,0.3)", borderTopColor: "#1a1a1a", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} /><span>Signing In...</span></>
              ) : (
                <><span>Sign In</span><ArrowRight size={13} strokeWidth={2.5} /></>
              )}
            </button>
          </div>
        </form>

        <p style={{ marginTop: "clamp(8px, 1.2vh, 14px)", textAlign: "center", fontFamily: "'Inter', sans-serif", fontSize: "12px", color: "#5f5e5e" }}>
          New to AstroAsk?{" "}
