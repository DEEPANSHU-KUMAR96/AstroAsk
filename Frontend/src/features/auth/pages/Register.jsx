...
const Register = () => {
  ...
  return (
    <div style={{ height: "100vh", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", background: "#fbf9f8" }}>
      <AtmosphericBg />
      <div className="glass-panel" style={{ width: "100%", maxWidth: "460px", borderRadius: "12px", padding: "clamp(20px, 3vh, 36px) clamp(24px, 3vw, 44px)", position: "relative", overflow: "hidden", zIndex: 1, margin: "0 16px" }}>
        ...
        <div style={{ marginBottom: "clamp(14px, 2.5vh, 24px)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" }}>
            ...
            <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(22px, 3.5vh, 36px)", fontWeight: 700, color: "#1a1a1a", lineHeight: 1.15, margin: 0 }}>Create Account</h1>
          </div>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "13px", lineHeight: 1.55, color: "#514532", margin: 0 }}>Begin your celestial journey. We require a few details to construct your astral profile.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate style={{ display: "flex", flexDirection: "column", gap: "clamp(12px, 2vh, 18px)" }}>
          ...
          <div style={{ paddingTop: "clamp(4px, 1vh, 10px)" }}>
            <button type="submit" disabled={loading} className="btn-gold" style={{ width: "100%", padding: "14px 24px" }} aria-busy={loading}>
              {loading ? (
                <><span style={{ display: "inline-block", width: "14px", height: "14px", border: "2px solid rgba(26,26,26,0.3)", borderTopColor: "#1a1a1a", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} /><span>Sending OTP...</span></>
              ) : (
                <><span>Continue to Alignment</span><ArrowRight size={14} strokeWidth={2.5} /></>
              )}
            </button>
          </div>
        </form>

        <p style={{ marginTop: "clamp(10px, 1.5vh, 16px)", textAlign: "center", fontFamily: "'Inter', sans-serif", fontSize: "13px", color: "#5f5e5e" }}>
          Already an initiate?{" "}
